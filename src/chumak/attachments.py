...
class Attachment(BaseModel):
    ...
    def _resolve_and_check_mime(self) -> Attachment:
        mime = self.mime
        if mime is None:
            mime, _ = mimetypes.guess_type(self.path.name)
            if mime is None:
                raise ValueError(
                    f"Attachment {str(self.path)!r}: cannot sniff MIME type from the file "
                    "extension; pass `mime=` explicitly"
                )
        if not mime.startswith("image/"):
            raise ValueError(
                f"Attachment {str(self.path)!r}: only image/* attachments are supported, "
                f"got {mime!r}"
            )
        self.mime = mime
        return self
